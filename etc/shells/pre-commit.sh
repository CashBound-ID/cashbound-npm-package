...
# Filter the listed packages to only include those located in the /packages/ or /pluggables/ directories
FILTERED_PACKAGE=$(echo "$ALL_PACKAGES" | jq -r '
  .[] |
  select(.path | contains("/packages/") or contains("/pluggables/")) |
  "\(.path) \(.name)"'
)
